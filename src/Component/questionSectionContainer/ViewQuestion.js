...
const ViewQuestion = () => {
  ...
  function fetchCall() {
    fetch(
      `${uri}/onlineexamapplication/control/view-questions?topicId=${topicId}`,
      { credentials: "include" }
    )
      .then((res) => res.json())
      .then((data) => {
    ...
    swalWithBootstrapButtons
      ...
      .then((result) => {
        if (result.isConfirmed) {
          fetch(
            `${uri}/onlineexamapplication/control/delete-question?questionId=${questionId}`,
            { credentials: "include" }
          )
            .then((res) => res.json())
            .then((data) => {
      ...
      {isAdmin ? (
        <div className="container ">
          <div className="card mt-5">
            <div
              className="card-header text-center text-light custom-card-header"
