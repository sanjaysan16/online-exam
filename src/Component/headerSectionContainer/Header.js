...
const Header = () => {
  ...
  async function logout() {
    ...
    const logoutFetch = await fetch(
      "https://localhost:8443/onlineexamapplication/control/user-logout",
      { credentials: "include" }
    );
            ...
            >
              <div className="offcanvass-body">
                <ul className="navbar-nav">
                  ...
                  <li>
                    <a
                      href={`dash-bord?partyIdOfAdmin=${partyIdOfAdminRef.current}`}
                    >
                  ...
                  <li>
                    <a
                      href={`view-user?partyIdOfAdmin=${partyIdOfAdminRef.current}`}
                    >
          ...
          <div className="col-md-4 col-sm-12 text-center">
            {refSetFlag.current ? (
              ...
              <div className="text-light p-2">
                welcome {name}
                <button className="p-2 btn btn-light" onClick={() => logout()}>
                  logout
                </button>
              </div>
            )}
