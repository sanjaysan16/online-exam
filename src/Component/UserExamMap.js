...
const UserExamMap = () => {
    ...
    var examListJsonData;
    useEffect(() => {
        const getExams = async () => {
            const examFetch = await fetch('https://localhost:8443/onlineexamapplication/control/get-exam-and-exam-list',{ credentials: "include" });
            const examListWithSlash = await examFetch.text();
            const examListwithoutSlash = examListWithSlash.substring(2);
            examListJsonData = JSON.parse(examListwithoutSlash);

            setExamList(examListJsonData._Exam_List_);
        }
        getExams();
    }, []);
