...
const UserExamMap = () => {
    ...
    const getExams = async () => {
        const examListFetch = await fetch('https://localhost:8443/onlineexamapplication/control/get-exam-or-exam-list',{ credentials: "include" });
        const examListJsonData = await examListFetch.json();
        setExamList(examListJsonData.Exam_List);
    }
    const getUserExamMappingList = async () => {
        const examFetch = await fetch('https://localhost:8443/onlineexamapplication/control/get-user-exam-mapping',{ credentials: "include" });
